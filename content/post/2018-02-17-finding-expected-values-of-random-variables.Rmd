...
In the process of writing up my answer, I wound up applying some functional programming techniques (specifically [function factories](https://adv-r.hadley.nz/functional-programming.html#function-factories9)). I also ended up exploring the problem quite far outside the scope of the original question, so I thought the full story would make more sense as a blog post -- so here we are!
...
Well, we _eventually_ made it here! The point is that based on the formula presented earlier in the [maths bit](#maths), we can use `integrate` to find the expected value of a random variable, or a transformation of one, given the PDF of the random variable.
...
And then try out the implementation on some simple distributions:

```{r}
find_mean(dexp, rate = 2)
```

But it could also be useful to generalise a bit, and create a _function factory_ instead. The idea is to make a function that, given a transformation function, will return another function that finds the expected value of the transformation of a random variable:
...
Since we know that finding moments can be seen as a special case of expected values of transformations, we can wrap `ev_finder` here to define _another_ function factory, this time for easy generation of functions to find moments.
